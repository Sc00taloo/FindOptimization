...
class GeneticAlgorithm:
    def __init__(self, func, generations=50, min=True, mut_chance=0.8, survive_cof=0.8, individuals=100):
        self.func = func
    ...
    #Функция, которая ранжирует особи в популяции и выбирает лучших родителей для скрещивания.
    def select(self):
        sorted_pop = dict(sorted(self.population.items(), key=lambda item: item[1][2], reverse=self.min_func))  # Ранжирование
        ...
        for pop in sorted_pop.values():
            if random() > 0.5:
                pop[0] = parents1[i][1][0] #pop[0] и pop[1] - координаты особи в пространстве по оси X и Y
                pop[1] = parents2[i][1][1]
                pop[2] = self.func(parents1[i][1][0], parents2[i][1][1]) #pop[2] - значение функции цели, вычисленное для данной особи.
            else:
    ...
    #Производит мутацию особей путем изменения значений координат с учетом вероятности мутации и номера текущего поколения.
    def mutation(self, cur_gen):
        for pop in self.population.values():
            ...
            pop[2] = self.func(pop[0], pop[1]) #pop[2] - значение функции цели, вычисленное для данной особи.